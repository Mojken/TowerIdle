			...
			if (main.w.keyboardListener.getCurrentlyPressedKeys()[KeyEvent.VK_SPACE][0] == 1 && shot == 0) {
				...
				for (Building b:Main.currentTrack.buildings) {
					if (b instanceof Block && ((Block) b).tower != null) {
						...
						p.shoot(Main.currentTrack.grid.getGridCoordinate(source.gridIndex).add(Main.currentTrack.tileSize.toVector().multiply(.5f)), source, target);
					}
				}
				shot = 1;
			}
	...
	public void render(java.awt.Graphics g2) {
		if (Main.initialized) {
			...
			for (Projectile p:Projectile.projectiles) {
				try {
					if(null == p)continue;
					g.drawImage(ImageIO.read(p.textureURL), p.pos);
				} catch (IOException e) {
